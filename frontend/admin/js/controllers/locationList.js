app.controller('LocationListCtrl', function ($scope, $http, $window) {
    $scope.locations = [];    // Start off with empty clients array
    $scope.pages = [];
    ...
    $scope.handleAction = function(action, id) {
        switch(action) {
            ...
            case "edit":
                $window.location.href = "#!/locations/detail/" + id;
                break;
    ...
    $scope.doDelete = function(user_id) {
        ...
        $http({
            method: 'POST',
            url: myndie.apiURL + "location/delete",
            data: params,
            headers: {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'},
        }).success(function (data) {
            if(!data.status) {
